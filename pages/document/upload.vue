<template>
  <a-card class="shadow">
    <div class="h1">{{ $t("create_document") }}</div>
      ...
      <a-row class="mt-2" v-if="!loading">
        <a-col v-bind="wrapperContainerCol">
          <a-form-model ref="modal-upload-document" v-bind="formLayout" :model="formUpload" :rules="getRules">
            ...
            <a-form-model-item :colon="false">
              <label slot="label" style="display: none"> a </label>
              <a-button class="mt-2" type="primary" icon="save" :loading="updating" @click="onUpload">{{
                $t("save")
              }}</a-button>
            </a-form-model-item>
          </a-form-model>
...
export default {
  ...
  computed: {
    ...
    documentNumber() {
      return this.formUpload.documentNumber
        ? this.formUpload.documentNumber
  ...
  methods: {
    ...
    async onUpload() {
      ...
      formUpload.description = this.formUpload.description ? this.formUpload.description : "";
      this.updating = true;
  ...
  async created() {
    ...
    if (this.$route.params.docId) {
      await this.getDocumentById(this.$route.params.docId);
      this.formUpload = {
        specId: this.item.spec_id,
        title: this.item.title,
        description: this.item.description,
        url: this.item.url,
        listTags: this.item.listTags,
        documentNumber: this.item.doc_number,
