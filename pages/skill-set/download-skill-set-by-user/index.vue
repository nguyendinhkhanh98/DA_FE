<template>
  <a-card>
    <a-row>
      <a-col :span="16">
        <span class="h1">{{ $t("download_skill_set_by_user") }}</span>
      </a-col>
          ...
          <tr>
            ...
            <th class="ant-table-column-has-actions ant-table-column-sort">
              <div style="display: inline-block">
                ...
                >
                  <a-select-option
                    v-for="(item, index) in listUsers.filter(user => !user.delete_flag)"
                    :key="index"
...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      exportSkillSetByUserId: "modules/skill-set/exportSkillSetByUserId"
    }),
    async handleExportSumarryReport() {
      this.loading = true;
      try {
        let response = await this.exportSkillSetByUserId({ selected_id: this.selectedRowKeys });
