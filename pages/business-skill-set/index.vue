<template>
  <a-card class="shadow">
    <div>
      <span class="h1" style="display: inline-block">{{ $t("list_assessment_business_skill") }}</span>
      ...
      <span class="ml-2 right-info" v-if="isLeader" style="display: inline-block">
        ...
        <a-button type="link" @click="createAssessment" style="display: inline-block"
          >{{ $t("create_assessment_business_skill") }} <a-icon type="caret-right"
        /></a-button>
      </span>
    </div>
    <a-spin :tip="`${$t('loading')}...`" :spinning="loading" class="w-100 h-100">
      ...
      >
        ...
        <template slot="detail" slot-scope="text">
          <a-button type="link" @click="goToUserDetail(text)"
            >{{ $t("detail") }} <a-icon type="caret-right"
          /></a-button>
        </template>
      </a-table>
    </a-spin>
...
export default {
  ...
  computed: {
    ...mapState({
      listUserPeriod: state => state.modules["business-skill"].listUserPeriod,
  ...
  async created() {
    ...
    promises.push(this.FetchAllUserPeriod());
  ...
  methods: {
    ...
    ...mapActions({
      FetchAllUserPeriod: "modules/business-skill/FetchAllUserPeriod",
    ...
    goToUserDetail(text) {
      this.$router.push(`/business-skill-set/${text}`);
    },
    createAssessment() {
      this.$router.push(`/business-skill-set/add`);
    },
    ...
    filterDataByStatus(clone) {
      if (this.dataFilter.filter.status && this.dataFilter.filter.status.length) {
        let statusReview = ["Waiting for review"];
