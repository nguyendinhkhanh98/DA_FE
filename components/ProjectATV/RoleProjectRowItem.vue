    ...
    <td v-for="column in columns" :key="column.id" class="category-item">
      ...
      <div v-if="column.key == 'detail'" style="text-align: end">
        ...
        <template v-else>
          ...
          >
            <a-button icon="undo" type="danger">{{ $t("restore") }}</a-button>
          </a-popconfirm>
        </template>
      </div>
      ...
      <span v-if="column.key == 'description'">
        ...
        <a-input v-else ref="input-edit-description" :defaultValue="record.description" style="width: auto" />
      </span>

      <span v-if="column.key == 'default_price'">
        <span v-if="!editting">
          {{ record[column.dataIndex] | numberFormatCurrency }}
        </span>
        <a-input-number v-else ref="input-edit-salary" v-model="record.monthly_salary" style="width: auto" :min="0" />
      </span>
    </td>
  </tr>
</template>
...
export default {
  ...
  methods: {
    ...
    async handleSaveRecord() {
      ...
      let vNodeInputDescription = this.$refs["input-edit-description"];

      let targetValue = Array.isArray(vNodeInput) ? vNodeInput[0].$el.value : vNodeInput.$el.value;
      let targetValueDescription = Array.isArray(vNodeInputDescription)
        ? vNodeInputDescription[0].$el.value
        : vNodeInputDescription.$el.value;
      ...
      if (this.mode == "create") {
        this.$emit("create", {
          ...
          monthly_salary: this.record.monthly_salary
        });
        return;
      }
